...
__global__ void sharpening_filter(unsigned char *img, unsigned char *omg, unsigned int width, unsigned int height, int channels) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  const float kernel[FILTER_HEIGHT][FILTER_WIDTH] = {
    {-1, -1, -1},
    {-1, 9, -1},
    {-1, -1, -1}
  };
    ...
    for (int channel = 0; channel < channels; ++channel) {
      ...
      for (int i = -FILTER_HEIGHT / 2; i <= FILTER_HEIGHT / 2; ++i) {
        for (int j = -FILTER_WIDTH / 2; j <= FILTER_WIDTH / 2; ++j) {
          sum += kernel[i + FILTER_HEIGHT / 2][j + FILTER_WIDTH / 2] * img[((y + i) * width + (x + j)) * channels + channel];
        }
      }
      omg[(y * width + x) * channels + channel] = sum;
    }
  }
}
...
void filter(Mat &img, Mat &omg) {
  ...
  sharpening_filter<<<grid, block>>>(d_img, d_omg, omg.cols, omg.rows, channels);
