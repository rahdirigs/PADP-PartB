...
__global__ void box_filter(unsigned char *img, unsigned char *omg, unsigned int width, unsigned int height, int channels) {
    ...
    for (int channel = 0; channel < channels; ++channel) {
      ...
      for (int i = -FILTER_HEIGHT / 2; i <= FILTER_HEIGHT / 2; ++i) {
        for (int j = -FILTER_WIDTH / 2; j <= FILTER_WIDTH / 2; ++j) {
          px += 1.0;
          sum += img[((y + i) * width + (x + j)) * channels + channel];
        }
      }
      omg[(y * width + x) * channels + channel] = sum / px;
    }
  }
}
...
void filter(Mat &img, Mat &omg) {
  ...
  box_filter<<<grid, block>>>(d_img, d_omg, omg.cols, omg.rows, channels);
