...
__global__ void laplacian_filter(unsigned char *img, unsigned char *omg, unsigned int width, unsigned int height) {
  ...
  if (filter_inside_image) {
    ...
    for (int i = -FILTER_HEIGHT / 2; i <= FILTER_HEIGHT / 2; ++i) {
      for (int j = -FILTER_WIDTH / 2; j <= FILTER_WIDTH / 2; ++j) {
        ...
        sum += value * kernel[i + FILTER_HEIGHT / 2][j + FILTER_WIDTH / 2];
      }
    }
    omg[y * width + x] = sum;
  }
...
void filter(Mat &img, Mat &omg) {
  ...
  laplacian_filter<<<grid, block>>>(d_img, d_omg, omg.cols, omg.rows);
